...
# In-text author-date citation metadata (APA style). Each entry gives
...
def build_tables_docx(tables, repl) -> Path:
    ...
    path = OUTPUT / "Tables_RTBM_editable.docx"
    doc.save(path)
    return path
...
def build_figures_pptx(figpaths, repl) -> Path:
    ...
    path = OUTPUT / "Figures_RTBM_editable.pptx"
    prs.save(path)
    return path
...
def main() -> None:
    ...
    print("  editable tables: output/Tables_RTBM_editable.docx")
    print("  editable figures: output/Figures_RTBM_editable.pptx")
